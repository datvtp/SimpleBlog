 ...
 * @author vanth
 */
public class AdminAcceptArticleController extends HttpServlet {

    private static final Logger logger = Logger.getLogger(AdminAcceptArticleController.class);
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (articleDAO.update(articleId, 2)) {
                url = "MainController?action=Search"
                        + "&txtSearch=" + searchTitle
                        + "&txtPageNumber=" + pageNumber
                        + "&txtStatusId=" + statusId;
            } else {
                request.setAttribute("ERROR", "Accept article failed.");
            }
        } catch (NumberFormatException e) {
            logger.error("ERROR NumberFormat at AdminAcceptArticleController: " + e.getMessage());
        } catch (SQLException e) {
            logger.error("ERROR SQL at AdminAcceptArticleController: " + e.getMessage());
        } catch (NamingException e) {
            logger.error("ERROR Naming at AdminAcceptArticleController: " + e.getMessage());
        } finally {
